...
public class SortedRecordsTableModel2 extends AbstractTableModel {

  private static final Logger LOG = Logger.getLogger(SortedRecordsTableModel2.class.getName());
  ...
  public SortedRecordsTableModel2(IMessenger messenger) {
    colHeaderDateFormat = new SimpleDateFormat("MM/dd/yyyy");
    cellDateFormat = new SimpleDateFormat("MM/dd HH:mm");
    try {
  ...
  public Object getValueAt(int rowIndex, int columnIndex) {
    ...
    if (attendanceData.eventTimes[rowIndex][columnIndex - baseColCount] == null) {
      return String.format("%.2f", val);
    }
    long[] eventTimes = attendanceData.eventTimes[rowIndex][columnIndex - baseColCount];
    return String.format("%.2f [%s - %s]", val, cellDateFormat.format(new Date(eventTimes[0])),
        cellDateFormat.format(new Date(eventTimes[1])));
  }
  ...
  private AttendanceData getAttendanceRecordsHoursPerDay(IMessenger messenger) throws IOException, UnknownUserException {
    ...
    for (AttendanceRecord record : recordList) {
      ...
      attendanceData.eventTimes[i] = record.getEventTimesByDay(timeStampMin, ndays);
